...
.ticker-move { 
  will-change: transform;          /* hint GPU for smoother animation */
  transform: translate3d(0,0,0);   /* force GPU acceleration */
  display: flex; 
  flex-shrink: 0; 
  animation: ticker-move 400s linear infinite;
}
...
@keyframes ticker-move {
  /* This will make the content scroll from right to left */
  from { transform: translate3d(0, 0, 0); }
  to { transform: translate3d(-100%, 0, 0); }
  /* Adjust the 'to' value based on whether the content is duplicated for a seamless loop */
}
...
- Non-matched but enrolled students are students who were matched to DOE data and enrolled but were later unmatched because a better matching student applied. These non-matched students still receive books. For more info see [here](/further_details.html).
- The "Enrolled %" calculates the percent of enrolled New Worlds Reading Students without including non-matched enrolled students.
...
disclaimer <- div(
  style = "
    ...
    padding-right: 10px;
  ",
  "*Disclaimer: The ticker is not 100% accurate due to delays in receiving data from partners. For the most up-to-date data regarding enrollments, it is best to reach out to the New Worlds Reading Data Team."
)

# put them in exact order: title → ticker → disclaimer → table
tagList(
  title,
  ticker,
  disclaimer,
