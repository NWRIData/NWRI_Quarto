...
#load data
report_data<-readRDS("report_data_schools.rds")
...
2.  There are `{r} nrow(orangesquare)` schools that have a lower than average enrollment and higher than average eligibility (Figure 2) and they make up `{r} 100 * round(nrow(orangesquare)/nrow(report_data),2)` % of all schools in the analysis. These schools on average have `{r} round(mean(orangesquare$Percent.of.Economically.Disadvantaged.Students),1)`% of their student enrollment classified as economically disadvantaged while the other schools average at `{r} round(mean(nonorangesquare$Percent.of.Economically.Disadvantaged.Students),1)`%.
...
ggplot(data = report_data) +
  ...
  labs(x = "Percent Economically Disadvantaged Students %",
       y = "Grades K-5 NWRI Eligibility", fill = "Percent ELL\nK-5 %") +
...
There are `{r} nrow(orangesquare)` schools that can be found in the orange square (Figure 3 & 4). Together they have `{r} sum(orangesquare$no_eligible_n)` students through grades K-5 who are eligible for NWRI. This represents `{r} 100 * round(percent_eligible_overtotal,2)`% of all eligible students in grades K-5 across the schools in this analysis. Most of these schools are found in `{r} maxcounty$NMCNTY` with `{r} maxcounty$total_n` eligible students across `{r} maxschoolno$n` schools. But the county with most students per school on average is `{r} avg_county_max$NMCNTY` at `{r} avg_county_max$avg` students. For more extensive data for each of these schools please use the interactive table below. Note: It is often easier to search for the county and then click it to view the schools and their statistics. You can sort the table by different columns as well.
