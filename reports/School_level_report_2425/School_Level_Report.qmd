...
actual_counts <- app_data %>%
  count(NMCNTY, SchName, appdate_month)

counts_filled <- grid %>%
  left_join(actual_counts, by = c("NMCNTY", "SchName", "appdate_month")) %>%
  mutate(n = replace_na(n, 0))

counts_filled<-counts_filled %>%
  filter(! appdate_month < "2024-06-01") %>%
  arrange(NMCNTY,SchName, appdate_month) %>%  # ensure chronological order
  group_by(NMCNTY,SchName) %>%
  summarise(counts = list(n)) %>%
  ungroup() %>%
  rename(County = NMCNTY)

school_summary2<-left_join(school_summary,counts_filled, by = c("County", "SchName"))  %>%
  left_join(.,report_data %>% select(NMCNTY,SchName,perc_eligible_k_5,perc_enroll_k_5),
            by = c("County" = "NMCNTY" ,"SchName" = "SchName"))
school_summary2<-school_summary2 %>%
  mutate(priorit_col = case_when(
    Priority == "Orange Square School" ~ "orange",
    TRUE ~ 'grey'
  )) %>%
  ...
  mutate(perc_eligible_k_5 = perc_eligible_k_5/100,
         perc_enroll_k_5 = perc_enroll_k_5/100) %>%
  select(-c(Priority, avg_perc, LOCALE)) %>%
  relocate(perc_eligible_k_5, .after =total_n) %>%
  relocate(perc_enroll_k_5, .after = perc_eligible_k_5) %>%
...
reactable(
  ...
  school_summary2,
  theme = nytimes(centered = TRUE, header_font_size = 12),
  
  groupBy = c("County", "School Type"),
  ...
  columns = list(
    ...
    SchName = colDef(name = "School",
                     cell = color_tiles(data = school_summary2, color_ref = 'priorit_col')),
    
    perc_enroll_k_5 = colDef(
      name = "Enrolled %",
    ...
    counts = colDef(
      name = "Application Activity for Enrolled Students",
      width = 250,
      sticky = "right",
      cell = react_sparkline(counts_filled,
                             line_color = "black",
                             tooltip_color = 'red',
                             line_width = 2,
                             line_curve = "linear",
  ...
  details = function(index) {
    school <- school_summary2$SchName[index]
