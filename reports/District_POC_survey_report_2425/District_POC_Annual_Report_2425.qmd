...
We generated word cloud based for each NPS group (Promoters, Detractors, and Passives). 
```{r, echo=FALSE, include=FALSE, warning=FALSE, cache=FALSE}
#generate word cloud by NPS group and save them for use with table

# Function to create word counts grouped by NPS group for a given question
make_word_list_by_group <- function(data, question_col, group_col = "Q4_NPS_GROUP") {
  data %>%
    select(all_of(c(group_col, question_col))) %>%
    rename(group = all_of(group_col), text = all_of(question_col)) %>%
    filter(!is.na(text)) %>%
    unnest_tokens(word, text) %>%
    anti_join(stop_words, by = "word") %>%
    count(group, word, sort = TRUE) %>%
    group_split(group) # returns a list, one df per group
}

save_word_clouds <- function(word_df, group_name, question_name) {
  safe_group <- gsub("[^a-zA-Z0-9]", "_", group_name)
  filename <- paste0("wordcloud_", question_name, "_", safe_group, ".png")
...
question_list <- c("Q5", "Q6", "Q7")

for (q in question_list) {
  message("Processing ", q)

  # Get word counts per group
  word_lists <- make_word_list_by_group(DIS_Data, q)

  # Match group names to word list (group_split returns data frames with attr "group")
  group_names <- unique(DIS_Data$Q4_NPS_GROUP)

  # Loop through and save each group’s word cloud
  walk2(word_lists, group_names, ~save_word_clouds(.x, .y, q))
}
