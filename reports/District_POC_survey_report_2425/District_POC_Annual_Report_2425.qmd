...
#also get a table for ELC and Districts
total_numbers<-DIS_Data %>%
  mutate(type = ifelse(str_detect(Q2, "ELC"), "ELC", "District")) %>%
  count(type) %>%
  drop_na()

types<-DIS_Data %>%
  mutate(type = ifelse(str_detect(Q2, "ELC"), "ELC", "District")) 
nrow(subset(types, type == "District"))
```
...
There was a total of `{r} length(unique(DIS_Data$ResponseId))` responders from `{r} total_numbers$n[total_numbers$type == "District"]` Districts and `{r} total_numbers$n[total_numbers$type == "ELC"]` ELCs. The overall NPS was `{r} NPS$NPS` while the customer satisfaction rate was `{r} cs_score$cust_perc`% [^1].

The NPS for District POC was `{r} NPS_NONELC$NPS ` and the customer satisfaction rate was `{r} cs_scorenonELC$cust_perc`% (sample size: `{r} nrow(subset(types, type == "District"))` responders).

The NPS for ELCs was `{r} NPS_ELC$perc ` and the customer satisfaction rate was `{r} cs_scoreELC$cust_perc`% (sample size: `{r} nrow(subset(types, type == "ELC"))` responders).

[^1]: We considered responses of 8-10 for the NPS questions as satisfied customers and calculated the percentage of those selecting 8-10 as the customer satisfaction percentage.
...
We generated word cloud based for each NPS group (Promoters, Detractors, and Passives) using all the answers across questions 5, 6, 7. 
...
combined_word_cloud<-DIS_Data %>%
  select(Q5, Q6, Q7) %>%
  pivot_longer(cols = c(Q5, Q6, Q7),names_to = "question") %>%
  mutate(value = str_replace_all(value, "NA", "")) %>%
  ...
  drop_na(word) %>%
   group_split(question) # returns a list, one df per group

attr(DIS_Data$Q5, "label")

#make three word clouds based on NPS group
group_names<-c("Q5", "Q6", "Q7")

save_word_clouds <- function(word_df, group_name) {
  safe_group <- gsub("[^a-zA-Z0-9]", "_", group_name)
  filename <- paste0("wordcloud", "_", safe_group, ".png")

  label <- attr(DIS_Data[[group_name]], "label")
  wrapped_title <- paste(strwrap(label, width = 35), collapse = "\n")
  ...
  wordcloud(
    ...
    freq = word_df$n,
    min.freq = 2, 
    max.words = 20,res=300,
    scale = c(4, 0.8),
    colors = brewer.pal(8, "Dark2"),
    random.order = FALSE
    )
   title(main = wrapped_title, cex.main = 2.5)
  dev.off()
...
<div id="wordcloud-scroll" style="overflow-x: auto; white-space: nowrap;">
  <img src="wordcloud_Q5.png" style="height: 400px; display: inline-block; margin-right: 10px;">
  <img src="wordcloud_Q6.png" style="height: 400px; display: inline-block; margin-right: 10px;">
  <img src="wordcloud_Q7.png" style="height: 400px; display: inline-block;">
</div>
...
js_filter <- JS(
  "function(rows, columnId, filterValue) {
     try {
       const pattern = filterValue.trim().toLowerCase().split(' ');
       return rows.filter(row => {
         const value = String(row.values[columnId] || '').toLowerCase();
         return pattern.every(term => value.includes(term));
       });
     } catch (e) {
       return rows;
     }
...
reactable(DIS_Data2,
          searchable = TRUE,highlight = TRUE,
            columns = list(
              Q2 = colDef(name = "County/ELC",  filterable = TRUE,
                          filterMethod = js_filter,
                          cell = js_match_style,
                          html = TRUE,
                          sortable = T),
              Q4_NPS_GROUP = colDef(name = "NPS Group"),
              Q5 = colDef(name = paste(attr(DIS_Data$Q5, "label")),
                          filterable = TRUE,
                          filterMethod = js_filter,
                          cell = js_match_style,
                          html = TRUE,
                          sortable = F),
              Q6 = colDef(name = paste(attr(DIS_Data$Q6, "label")),
                          filterable = TRUE,
                          filterMethod = js_filter,
                          cell = js_match_style,
                          html = TRUE,
                          sortable = F),
              Q7 = colDef(name = paste(attr(DIS_Data$Q7, "label")),
                          filterable = TRUE,
                          filterMethod = js_filter,
                          cell = js_match_style,
                          html = TRUE,
